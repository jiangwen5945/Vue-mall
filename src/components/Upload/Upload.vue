  ...
  <div id="upload">
    <div class="upload-box">
      ...
      <div class="upload-header">
        ...
        <div class="upload-header-close" @click="closePicBox">
      ...
      <div class="upload-content">
        <div class="upload-content-left">
          <div
            class="upload-content-left-item upload-content-left-upload"
            @click="menuSelect($event,'new')"
            :class="{'upload-content-left-item-active':group==='new'}"
          >上传图片</div>
          <div class="upload-content-left-item-wrapper">
            <div
              class="upload-content-left-item upload-content-left-group-item"
              @mouseenter="delGroupFlag = index"
              @mouseleave="delGroupFlag = ''"
              :class="{'upload-content-left-item-active':group===item.group}"
            ...
            >
              ...
              <span
                class="icon-cross"
                v-if="delGroupFlag === index && item.group!=='all'&& item.group!=='default'"
                @click="delGroup(item.group)"
              ></span>
            </div>
          </div>
        ...
        <div class="upload-content-right">
          ...
          <div class="upload-content-right-content" ref="picWrapper">
            ...
            <div class="upload-content-no-pics" v-if="noPic">暂无图片</div>
            <div
              class="upload-content-right-item-wrapper"
            ...
            >
              <div class="upload-content-right-item" @click="selectThis($event,item.id,index)">
                ...
                <div class="upload-progress" v-if="progressShow">
                  <div
                    ...
                    ref="progress"
                    :class="{'too-large':item.size>maxSize}"
                  ></div>
                </div>
                <!--选择标志-->
              ...
              <div
                class="upload-content-right-item-del"
                title="删除"
                @click="delPic(item.id)"
                v-if="group !== 'all'"
              >
    ...
    <div class="upload-modal" @click="closePicBox"></div>
  </div>
</template>
        ...
        let config = {
          ...
          onUploadProgress(progressEvent) {
            ...
            if (progressEvent.lengthComputable) {
              ...
              //进度条
              _this.$refs.progress[
                _this.$refs.progress.length - 1
              ].style.width = Number(progress).toFixed(2) * 100 + "%";
            }
          },
        };
        //向formData中插入文件
        if (file.files[count]) {
          ...
          fileReader.onloadend = () => {
            ...
            if (formData.get("file").size > _this.maxSize) {
              ...
              //发送数据
            //   axios
            //     .post(`/upload?mark=${_this.group}`, formData, config)
            //     .then((response) => {
            //       formData.delete("file");
            //       let res = response.data;
            //       console.log(res);
...
#upload
  .uploader-bottom
  .uploader-bottom-pagegroup
  .uploader-bottom-page-item-active {
...
#upload
  .upload-content
  .upload-content-left
  .upload-content-left-item.upload-content-left-group-item {
  display: flex;
  justify-content: space-between;
  align-items: center;
}
...
#upload
  ...
  .upload-content-right-item-del:hover {
  background: #3eb5dd;
}
#upload
  .upload-content
  .upload-content-right
  .upload-content-right-item-del
  .icon-cross {
  line-height: 20px;
}
#upload
  .upload-content
  .upload-content-right
  .upload-content-right-item-select-mark {
  width: 0;
  height: 0;
...
#upload
  .upload-content
  .upload-content-right
  .upload-content-right-item-select-mark:before {
  content: "";
  width: 8px;
  height: 3px;
  display: block;
...
#upload
  .upload-content
  .upload-content-right
  .upload-content-right-item-select-mark:after {
  content: "";
  width: 13px;
  height: 3px;
  display: block;
...
#upload
  .upload-content
  .upload-content-right
  .upload-content-right-item:hover
  img {
  opacity: 0.8;
}
